...
// zero
#pragma omp target teams distribute parallel for map(tofrom : zv[ : MAXVTX], wv[ : MAXVTX], chi2[ : MAXVTX])
...
#pragma omp target teams distribute parallel for map(to : iv[ : MAXTRACKS], zt[ : MAXTRACKS], ezt2[ : MAXTRACKS]) \
    map(tofrom : zv[ : MAXVTX], wv[ : MAXVTX])
...
// reuse nn
#pragma omp target teams distribute parallel for map(to : wv[ : MAXVTX]) map(tofrom : zv[ : MAXVTX], nn[ : MAXTRACKS])
  for (uint32_t i = 0; i < foundClusters; i++) {
    //if (!(wv[i] > 0.f))
...
// compute chi2
#pragma omp target teams distribute parallel for map(to : zt[ : MAXTRACKS], ezt2[ : MAXTRACKS], zv[ : MAXVTX]) \
    map(tofrom : chi2[ : MAXVTX], nn[ : MAXTRACKS], iv[ : MAXTRACKS])
  for (uint32_t i = 0; i < nt; i++) {
    if (iv[i] > 9990)
      continue;
...
#pragma omp target teams distribute parallel for map(to : nn[ : MAXTRACKS], chi2[ : MAXVTX]) map(tofrom : wv[ : MAXVTX])
  for (uint32_t i = 0; i < foundClusters; i++)
    if (nn[i] > 0)
