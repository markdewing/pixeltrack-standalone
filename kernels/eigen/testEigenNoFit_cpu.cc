...
// compile and link with -foffload-lto to avoid the link error with __assert_fail
// link error is also fixed in LLVM mainline (post 15)

// Get many errors about Eigen::Matrix
// Type 'Eigen::Matrix<double, 3, 3, 0>' is not trivially copyable and not guaranteed to be mapped correctly [-Wopenmp-mapping]
// Suppress those for this file
...
void kernel(Matrix3d *m, Eigen::SelfAdjointEigenSolver<Matrix3d>::RealVectorType *ret) { eigenValues(m, ret); }
#endif
...
//  Map<M3> res(result->data());
  ...
  //  res.noalias() = (*J) * (*C);
  //  printIt(&res);
#pragma omp target map(to : J[:1], C[:1]) map(from : result[:1])
  (*result) = (*J) * (*C);
#if TEST_DEBUG
  printf("*** GPU OUT ***\n");
#endif
...
#pragma omp target map(from : mCPUret[:1])
...
#pragma omp target map(from : ret1 [0:1])
  kernel(&m_gpu, ret1);
