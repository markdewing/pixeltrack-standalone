...
using HitContainer = pixelTrack::HitContainer;
 ...
 void kernel_checkOverflows(HitContainer const *foundNtuplets,
  ...
  uint32_t first = 0 + blockIdx.x * blockDim.x;
  ...
  for (int idx = first, nt = foundNtuplets->nbins(); idx < nt; idx += blockDim.x) {
  ...
  for (int idx = first, nt = (*nCells); idx < nt; idx += blockDim.x) {
  ...
  for (int idx = first, nt = nHits; idx < nt; idx += blockDim.x) {
 ...
 void kernel_connect(cms::cuda::AtomicPairCounter *apc1,
  ...
  auto firstCellIndex = 0 + blockIdx.y * blockDim.y;
  uint32_t first = 0;
  ...
  for (int idx = firstCellIndex, nt = (*nCells); idx < nt; idx += blockDim.y) {
    auto cellIndex = idx;
 ...
 void kernel_countMultiplicity(HitContainer const *__restrict__ foundNtuplets,
  ...
  for (int it = first, nt = foundNtuplets->nbins(); it < nt; it += blockDim.x) {
 ...
 void kernel_classifyTracks(HitContainer const *__restrict__ tuples,
  ...
  for (int it = first, nt = tuples->nbins(); it < nt; it += blockDim.x) {
 ...
 void kernel_doStatsForTracks(HitContainer const *__restrict__ tuples,
                                        ...
                                        CAHitNtupletGeneratorKernelsCPU::Counters *counters) {
  int first = blockDim.x * blockIdx.x + 0;
  for (int idx = first, ntot = tuples->nbins(); idx < ntot; idx += blockDim.x) {
 ...
 void kernel_fillHitDetIndices(HitContainer const *__restrict__ tuples,
  ...
  int first = blockDim.x * blockIdx.x + 0;
  // copy offsets
  for (int idx = first, ntot = tuples->totbins(); idx < ntot; idx += blockDim.x) {
    hitDetIndices->off[idx] = tuples->off[idx];
  }
  // fill hit indices
  auto const &hh = *hhp;
  auto nhits = hh.nHits();
  for (int idx = first, ntot = tuples->size(); idx < ntot; idx += blockDim.x) {
 ...
 void kernel_doStatsForHitInTracks(CAHitNtupletGeneratorKernelsCPU::HitToTuple const *__restrict__ hitToTuple,
  ...
  auto &c = *counters;
  int first = blockDim.x * blockIdx.x + 0;
  for (int idx = first, ntot = hitToTuple->nbins(); idx < ntot; idx += blockDim.x) {
 ...
 void kernel_print_found_ntuplets(TrackingRecHit2DSOAView const *__restrict__ hhp,
  ...
  auto const &tracks = *ptracks;
  int first = blockDim.x * blockIdx.x + 0;
  for (int i = first, np = std::min(maxPrint, foundNtuplets.nbins()); i < np; i += blockDim.x) {
