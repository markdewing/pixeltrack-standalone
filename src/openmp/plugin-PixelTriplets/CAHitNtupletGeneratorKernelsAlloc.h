...
void CAHitNtupletGeneratorKernelsCPU::allocateOnGPU(cudaStream_t stream) {
  ...
  device_storage_ = Traits::template make_unique<cms::openmp::AtomicPairCounter::c_type[]>(3, stream);

  device_hitTuple_apc_ = (cms::openmp::AtomicPairCounter*)device_storage_.get();
  device_hitToTuple_apc_ = (cms::openmp::AtomicPairCounter*)device_storage_.get() + 1;
  device_nCells_ = (uint32_t*)(device_storage_.get() + 2);

  *device_nCells_ = 0;
  cms::openmp::launchZero(device_tupleMultiplicity_.get(), stream);
  cms::openmp::launchZero(device_hitToTuple_.get(), stream);  // we may wish to keep it in the edm...
}
