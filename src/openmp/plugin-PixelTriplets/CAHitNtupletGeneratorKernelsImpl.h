...
using HitContainer = pixelTrack::HitContainer;
 ...
 void kernel_connect(cms::openmp::AtomicPairCounter *apc1,
                               cms::openmp::AtomicPairCounter *apc2,  // just to zero them,
 ...
 void kernel_print_found_ntuplets(TrackingRecHit2DSOAView const *__restrict__ hhp,
  ...
  auto const &tracks = *ptracks;
  int first = 0;
  for (int i = first, np = std::min(maxPrint, foundNtuplets.nbins()); i < np; i++) {
