...
void CAHitNtupletGeneratorKernelsCPU::launchKernels(HitsOnCPU const &hh, TkSoA *tracks_d, cudaStream_t cudaStream) {
  ...
  // zero tuples
  cms::openmp::launchZero(tuples_d, cudaStream);
  ...
  cms::openmp::finalizeBulk(device_hitTuple_apc_, tuples_d);
  ...
  cms::openmp::launchFinalize(device_tupleMultiplicity_.get(), cudaStream);
...
void CAHitNtupletGeneratorKernelsCPU::classifyTuples(HitsOnCPU const &hh, TkSoA *tracks_d, cudaStream_t cudaStream) {
  ...
  cms::openmp::launchFinalize(device_hitToTuple_.get(), cudaStream);
