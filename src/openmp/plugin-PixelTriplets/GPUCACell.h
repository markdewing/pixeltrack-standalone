...
public:
  ...
  inline void init(CellNeighborsVector& cellNeighbors,
    ...
    CellTracks& tracks() { return *theTracks; }
    CellTracks const& tracks() const { return *theTracks; }
    CellNeighbors& outerNeighbors() { return *theOuterNeighbors; }
    CellNeighbors const& outerNeighbors() const { return *theOuterNeighbors; }
    float get_inner_x(Hits const& hh) const { return hh.xGlobal(theInnerHitId); }
    float get_outer_x(Hits const& hh) const { return hh.xGlobal(theOuterHitId); }
    float get_inner_y(Hits const& hh) const { return hh.yGlobal(theInnerHitId); }
    float get_outer_y(Hits const& hh) const { return hh.yGlobal(theOuterHitId); }
    float get_inner_z(Hits const& hh) const { return theInnerZ; }
  // { return hh.zGlobal(theInnerHitId); } // { return theInnerZ; }
    float get_outer_z(Hits const& hh) const { return hh.zGlobal(theOuterHitId); }
    float get_inner_r(Hits const& hh) const { return theInnerR; }
  // { return hh.rGlobal(theInnerHitId); } // { return theInnerR; }
    float get_outer_r(Hits const& hh) const { return hh.rGlobal(theOuterHitId); }

    auto get_inner_iphi(Hits const& hh) const { return hh.iphi(theInnerHitId); }
    auto get_outer_iphi(Hits const& hh) const { return hh.iphi(theOuterHitId); }

    float get_inner_detIndex(Hits const& hh) const { return hh.detectorIndex(theInnerHitId); }
    float get_outer_detIndex(Hits const& hh) const { return hh.detectorIndex(theOuterHitId); }
