  ...
  // Kernel to perform Raw to Digi conversion
   ...
   void fillHitsModuleStart(uint32_t const *__restrict__ cluStart, uint32_t *__restrict__ moduleStart) {
    ...
    for (int i = first, iend = gpuClustering::MaxNumModules; i < iend; i++) {
     ...
     uint32_t ws[32];
    cms::openmp::blockPrefixScan(moduleStart + 1, moduleStart + 1, 1024, ws);
    cms::openmp::blockPrefixScan(moduleStart + 1025, moduleStart + 1025, gpuClustering::MaxNumModules - 1024, ws);
