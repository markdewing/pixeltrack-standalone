...
namespace gpuClustering {
  ...
  uint32_t gMaxHit = 0;
   ...
   void countModules(uint16_t const* __restrict__ id,
                               ...
                               int numElements) {
    int first = 0;
#pragma omp target teams distribute parallel for map(tofrom:moduleStart[:MaxNumModules+1],clusterId[:numElements]) map(to:id[:numElements])
...
#pragma omp critical
      ...
      using Hist = cms::openmp::HistoContainer<uint16_t, nbins, maxPixInModule, 9, uint16_t>;
       Hist hist;
      ...
      for (int i = first; i < msize; i++) {
        ...
        hist.count(y[i]);
#ifdef GPU_DEBUG
        // Atomic add fails at runtime with:
